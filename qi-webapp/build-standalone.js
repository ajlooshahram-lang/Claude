...
async function main() {
  ...
  // 3) inline the self-hosted vendor libraries (no network needed)
  const libBlocks = [];
  for (const name of VENDOR) {
    const code = fs.readFileSync(path.join(ROOT, "vendor", name), "utf8");
    libBlocks.push("<script>/* vendor/" + name + " */\n" + safeScript(code) + "\n</script>");
    process.stdout.write("  inlined vendor/" + name + " (" + code.length + " bytes)\n");
  }
  ...
  const firstScript = html.indexOf('<script src="vendor/chart.umd.min.js"');
